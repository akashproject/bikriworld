...
<div class="col-12">
	<div class="card">
		<form class="form-horizontal" method="post" action="{{ url('submit-product-config-price') }}" enctype="multipart/form-data">
			@csrf
			<div class="card-body">
				<h4 class="card-title"> Edit Product Configuration Price </h4>
				@if ($errors->any())
				...
				<div class="form-group row">
					...
					<div class="col-sm-9">
						<select name="config_id" id="config_id" class="select2 form-control custom-select" style="width: 100%;" >
							...
							<option value="{{$config->id}}"   {{ ( $productConfigPrice->config_id ==  $config->id )? 'selected' : '' }} > {{$config->value}}</option>
							@endforeach
						<select>
					</div>
				</div>
				...
				<div class="form-group row">
					...
					<div class="col-sm-9">
						<select name="product_id" id="product_id" class="select2 form-control custom-select" style="width: 100%;">
							...
							<option value="{{$product->id}}" {{ ( $productConfigPrice->product_id ==  $product->id )? 'selected' : '' }}> {{$product->name}}</option>
							@endforeach
						<select>
					</div>
				</div>
				...
				<div class="form-group row">
					...
					<div class="col-sm-9">
						<input type="text" class="form-control" name="price" id="price" placeholder="Price Here" value="{{ $productConfigPrice->price }}" >
					</div>
				</div>
			</div>
			...
			<div class="border-top">
				...
				<div class="card-body">
					...
					<input type="hidden" name="product_config_price_id" id="product_config_price_id" value="{{ $productConfigPrice->id }}" >
				</div>
